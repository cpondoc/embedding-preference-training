...
def download_pages():
    ...
    with open("data/common-crawl/cc-final.txt", 'r') as f:
        ...
        for _, line in enumerate(f):
            ...
            unzip_index("data/common-crawl/cc-pages/" + crawl_path[-12:], "data/common-crawl/cc-txt/" + str(count) + ".txt") 
            count += 1
            if count == 100:
                return
        ...
        for record in ArchiveIterator(stream):
            ...
            if record.rec_type == 'response':
                ...
                try:
                    ...
                    main_text = BeautifulSoup(main_html, 'lxml').get_text()  # Extract plain text
                    
                    # Print the URL and the core text (or save to file)
                    # print(f"URL: {url}\nMain Text:\n{main_text}\n\n")
                    # print(f"URL: {url}")
                    # Alternatively, save content to a file if needed:
                    with open(f"data/common-crawl/sample-data/{str(count)}.txt", "w+") as output_file:
                        count += 1
